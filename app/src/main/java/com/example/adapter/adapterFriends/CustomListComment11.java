...
public class CustomListComment11 extends BaseAdapter {
	...
	public CustomListComment11(Context c, Model_Comment m) {
		mContext = c;
		this.m = m;
	...
	protected void removeItemFromList(final int position) {
		...
		alert.setPositiveButton("YES", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				try {
					commentdlt(Json_commentlist_fp.model_comment.getAl_comment().remove(position).getId());
				}
				catch (Exception e){}
...
try {
	new Json_commentlist_fp(mContext, NewComment_FP.list).execute(CustomGridProduct2.pid, "p");
}
catch (Exception e){
					...
					int tv=Integer.parseInt(String.valueOf(NewComment_FP.tv_comment.getText()));
					value=value+tv;



					NewComment_FP.tv_comment.setText(String.valueOf(value));
