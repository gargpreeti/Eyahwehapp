...
public class CustomListComment12 extends BaseAdapter {
   ...
   public  static Button btn_cmt;
	...
	public CustomListComment12(Context c, Model_Comment m) {
...
try {
	if (Json_commentlist_fs.model_comment.getAl_comment().get(position).getUserid().equals(FragmentHome.userid)) {
	...
	protected void removeItemFromList(final int position) {
		...
		alert.setPositiveButton("YES", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				try {
					commentdlt(Json_commentlist_fs.model_comment.getAl_comment().remove(position).getId());
		...
		class Commentdlt extends AsyncTask<String, Void, String> {
			...
			protected void onPostExecute(String s) {
				...
				if(result2.equals("true")) {
					...
					new Json_commentlist_fs(mContext, NewComment_FS.list).execute(CustomGridService2.sid, "s");

					String val="-1";
					int value=Integer.parseInt(val);
					int tv=Integer.parseInt(String.valueOf(NewComment_FS.tv_comment.getText()));
					value=value+tv;


					NewComment_FS.tv_comment.setText(String.valueOf(value));
